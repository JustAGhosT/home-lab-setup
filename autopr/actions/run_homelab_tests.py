...
class RunHomelabTests(Action[Inputs, Outputs]):
    ...
    async def run(self, inputs: Inputs) -> Outputs:
                ...
                if line.strip().startswith("-") and ": " in line:
                    ...
                    if current_test:
                        failed_test_details.append(
                            FailedTest(
                                name=current_test, error_message=current_error.strip()
                            )
                        )
        ...
        if current_test:
            failed_test_details.append(
                FailedTest(name=current_test, error_message=current_error.strip())
            )

        # Look for CommandNotFoundException errors
        command_not_found_match = re.search(
            r"CommandNotFoundException: Could not find Command ([\w-]+)", stdout_text
        )
...
if __name__ == "__main__":
    ...
    asyncio.run(
        ...
        run_action_manually(
            ...
            inputs=Inputs(
                test_type="Unit", coverage=True, generate_report=True, fix_issues=True
            ),
        )
    )
