function Select-GitHubRepository {
    ...
    try {
        # Main loop to handle repository selection and refresh
        do {
            # Get repositories
            $params = @{
                Type = $Type
            ...
            do {
                ...
                if ($selection -eq $refreshOption -or $selection -eq 'r' -or $selection -eq 'refresh') {
                    ...
                    break  # Break out of the inner selection loop to refresh
                }
        ...
        } while ($true)  # Outer loop for refresh functionality
    }
    catch {
